...
namespace BookStore
{
    public partial class MainForm : Form
    {
        ...
        private void импортExcelToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            openFileDialog.Filter = "CSV файлы (*.csv)|*.csv|Текстовые файлы (*.txt)|*.txt";
            openFileDialog.Title = "Выберите CSV файл с книгами";
            ...
            if (openFileDialog.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    ...
                    for (int i = 1; i < lines.Length; i++)
                    {
                        ...
                        {
                            DataTable dtAuthor = db.ExecuteQuery("SELECT AuthorID FROM Authors WHERE LastName + ' ' + FirstName LIKE @name",
                                new SqlParameter[] { new SqlParameter("@name", "%" + author + "%") });
                        ...
                        {
                            DataTable dtPublisher = db.ExecuteQuery("SELECT PublisherID FROM Publishers WHERE PublisherName = @name",
                                new SqlParameter[] { new SqlParameter("@name", publisher) });
                            if (dtPublisher.Rows.Count > 0)
                                publisherId = Convert.ToInt32(dtPublisher.Rows[0]["PublisherID"]);
                            else
                            {
                                string insertPublisher = "INSERT INTO Publishers (PublisherName) VALUES (@name); SELECT SCOPE_IDENTITY();";
                                publisherId = Convert.ToInt32(db.ExecuteScalar(insertPublisher, new SqlParameter[] { new SqlParameter("@name", publisher) }));
                            }
                        }
                        ...
                        string insertBook = @"INSERT INTO Books (Title, AuthorID, PublisherID, Price, QuantityInStock) 
                                      VALUES (@title, @authorId, @publisherId, @price, @qty)";
                        db.ExecuteNonQuery(insertBook, new SqlParameter[] {
                    ...
                    MessageBox.Show($"Импортировано {importedCount} книг!");
                    LoadBooks();
                }
                catch (Exception ex)
                {
        ...
        private void экспортВExcelToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            DialogResult result = MessageBox.Show(
                "Экспортировать книги или заказы?\n\nДа - книги, Нет - заказы, Отмена - оба файла",
                "Выбор экспорта",
            ...
            {
                ExportHelper.ExportToCsv(dgvBooks, "Книги");
            }
            else if (result == DialogResult.No)
            {
                ExportHelper.ExportToCsv(dgvOrders, "Заказы");
            }
            else if (result == DialogResult.Cancel)
            {
                ExportHelper.ExportBooksAndOrders(dgvBooks, dgvOrders);
            }
        }
